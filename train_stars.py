...
def main(epochs, prop_labeled=0.1, mechanism="car"):
    """Train an SSL model"""
    logging.basicConfig(
        level=logging.INFO,
    ...
    train_ds, test_ds = load_stars(prop_labeled, mechanism)
    in_shape = (6,)
    out_units = 3
    ...
    model_sl.save(f"data/models/stars-{mechanism}-{prop_labeled}-toy_nn-sl.keras")
    ...
    model_pi.save(f"data/models/stars-{mechanism}-{prop_labeled}-toy_nn-pi.keras")
    ...
    model_te.save(f"data/models/stars-{mechanism}-{prop_labeled}-toy_nn-te.keras")


if __name__ == "__main__":
    os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"  # FATAL
    logging.getLogger("tensorflow").setLevel(logging.FATAL)
    main(epochs=10, prop_labeled=0.25, mechanism="ent")
